...
export const WhyChooseUs: React.FC<WhyChooseUsProps> = ({ onOpenQuoteModal }) => {
  ...
  return (
    <section id="why-us" className="py-16 md:py-24 bg-slate-900 text-white relative overflow-hidden">
      {/* Background accents */}
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-orange-400 text-xs font-bold uppercase tracking-wider mb-3">
            Our Key Advantages
          </div>
          ...
          <p className="mt-4 text-base text-slate-300">
            Backed by deep knowledge of industrial requirements and a prime location at Mathura Refinery Main Gate, we offer dependable safety solutions tailored to your operations.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {points.map((pt, idx) => {
            ...
            return (
              <div
                key={pt.id}
                className="bg-slate-800/80 rounded-2xl p-6 border border-slate-700/80 hover:border-orange-500/80 transition-all duration-300 hover:-translate-y-1 shadow-lg flex flex-col justify-between group"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <div className="w-12 h-12 rounded-xl bg-orange-500/15 border border-orange-500/30 text-orange-400 flex items-center justify-center group-hover:bg-orange-500 group-hover:text-white transition-colors">
                      <IconComp className="w-6 h-6" />
                    </div>
                    <span className="text-[10px] font-bold text-amber-400 bg-slate-900/90 px-2 py-0.5 rounded border border-slate-700">
                      {pt.badge}
                    </span>
                  </div>
                ...
                <div className="mt-6 pt-4 border-t border-slate-700/60 flex items-center gap-1.5 text-xs text-slate-400">
        ...
        <div className="mt-12 p-6 rounded-2xl bg-gradient-to-r from-slate-800 to-slate-800/90 border border-slate-700 grid grid-cols-1 md:grid-cols-3 gap-6 text-center md:text-left">
          <div className="flex items-center gap-3">
            <Truck className="w-8 h-8 text-orange-400 shrink-0 mx-auto md:mx-0" />
            <div>
          ...
          <div className="flex items-center gap-3">
            <Award className="w-8 h-8 text-amber-400 shrink-0 mx-auto md:mx-0" />
            <div>
